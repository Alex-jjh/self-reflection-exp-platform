...
>   ITEM_RETRIEVAL_GUIDE ①），回译推荐不阻塞——问卷条目层面可上线
...
> **✅ 已裁决（Alex 07-27）：定稿为混合来源 15 条（岔路 A）**——9 条 ⚠
> 不再追加验证来源，limitation 承认 AI 辅助翻译（定稿措辞见
> ITEM_RETRIEVAL_GUIDE ①）；回译降级为推荐不阻塞。】
...
- GIH-6：均分，高=更谦逊。Dweck-3：反向计分后均分，高=成长型。
  - ⚠ **测量时机护栏（2026-07-30 文献扫荡 L_2603.23855 触发）**：GIH-6
    仅作**一次性 pre-study 筛选协变量**用，**不做 session 前后测**——这是
    刻意的。原因：一次 12 分钟的结构化反思式 LLM 对话就能显著抬升 GIH
    并持续 14 天（g=0.46→0.41，无衰减；无对照条件下 GIH 是 r=.90 的稳定
    特质）。**若未来任何版本改成绕反思 session 前后测 GIH，其漂移会混淆
    "我们 RQ3 的 supportive/challenging/neutral 操纵"与"泛化的'跟 LLM 做了
    结构化反思对话'效应"**——届时须设对照或在 limitation 明写。当前设计
    不受影响，此条为未来迭代留痕。
