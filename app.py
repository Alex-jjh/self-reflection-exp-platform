...
def init_session(participant_id: str):
    ss = st.session_state
...
with st.sidebar:
    st.caption("主持人视图")
    st.write(f"参与者：{ss.participant_id}")
...
if ss.session_done:
    st.success("三个对话都完成了，谢谢！请回到主持人处继续。")
    st.stop()

# --- post-episode ratings (3 items, R4) ---
if ss.ratings_pending:
    st.subheader("请为刚才这段对话打分")
    r1 = st.slider("这个AI聪明吗？", 1, 7, 4)
    r2 = st.slider("这个AI懂你吗？", 1, 7, 4)
    r3 = st.slider("这段对话有帮助吗？", 1, 7, 4)
    if st.button("提交，进入下一段"):
...
if ss.display and ss.display[-1][0] == "assistant":
    if st.button("重新生成", key=f"regen_{len(ss.display)}", type="tertiary"):
